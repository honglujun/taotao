...
<body id="mainframe">
	...
	<form id="orderForm" class="hide">
		...
		<input type="hidden" name="orderItems[0].num" value="1" /> <input
			type="hidden" name="orderItems[0].price" value="${item.price}" /> <input
			type="hidden" name="orderItems[0].totalFee" value="${item.price}" />
		<input type="hidden" name="orderItems[0].title" value="${item.title}" />
		<input type="hidden" name="orderItems[0].picPath"
			value="${item.image}" /> <input type="hidden" name="payment"
		...
		<input type="hidden" name="orderShipping.receiverMobile"
			...
			name="orderShipping.receiverState" value="上海" /> <input
			type="hidden" name="orderShipping.receiverCity" value="上海" /> <input
			type="hidden" name="orderShipping.receiverDistrict" value="闵行区" /> <input
					...
					<div class="step-content">
						<div id="consignee" class="sbox-wrap">
							<div class="sbox">
								<div class="s-content">
									<p>
										收货人&nbsp;&nbsp;：${my-order.orderShipping.receiverName} &nbsp;
										<br /> 手机电话：${my-order.orderShipping.receiverMobile } <br />
										地&nbsp; &nbsp;&nbsp;址：${my-order.orderShipping.receiverState }
										${my-order.orderShipping.receiverCity }
										${my-order.orderShipping.receiverDistrict } &nbsp;
										${my-order.orderShipping.receiverAddress } 三鲁公路3279号 明浦广场 3号楼
										205室
									</p>
								</div>
							</div>
							...
							<div class="sbox">
								<div class="s-content payment-info">
									<div class="payment-selected">
										<c:choose>
											<c:when test="${ oderMore.status == 1}">在线支付</c:when>
											<c:when test="${ oderMore.status == 2}">货到付款</c:when>
											...
											<table class="review-thead">
												...
												<div class="review-tbody">
													<table class="order-table">
														<tbody>
														<c:forEach items="${my-order.orderItems }" var="om">
															<tr>
																<td class="fore1">
																	<div class="p-goods">
																		<div class="p-img">
																			<a href="http://www.taotao.com/item/${om.orderId}.html"
																...
																<td class="p-price"><strong>￥${om.price/1000 }<fmt:formatNumber
																			groupingUsed="false" maxFractionDigits="0"
																			minFractionDigits="0" value="${item.price /100 }" /></strong>
																</td>
																<td class="p-promotion">
																&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;￥${om.price/1000 }
																	
																</td>
																<td class="fore2">x${om.num }</td>
																<td class="fore2 p-inventory" skuId="1057746">
																<c:if test="${om.num >0}">
																<p>有货</p>
																</c:if>
																<c:if test="${om.num <1}">
...
.checkout-buttons .checkout-submit-disabled b {
	...
	cursor: not-allowed;
								...
								<!--input type="
															submit"  class="checkout-submit" value=""
                                      	  ...
                                      	  提交订单
                                        <b></b>
                                </button> --> <span class="total">应付总额：<strong
																id="payPriceId">￥<fmt:formatNumber
																		groupingUsed="false" maxFractionDigits="2"
															...
															<div style="display: none"
																id="factoryShipCodShowDivBottom" class="dispatching">部分商品货到付款方式：先由淘淘配送“提货单”并收款，然后厂商发货。</div>
															</div> <span id="submit_message" style="display: none"
															class="submit-error"></span>
															<div class="submit-check-info"
																id="submit_check_info_message" style="display: none"></div>
															</div>
															</div>
															</div>
															</div>
															</div>
															</div> <!-- freight 弹窗,不放到orderInfo中弹框 就会串行-->
															...
															<div class="w">
																...
																type="text/javascript" src="/js/base-2011.js"></script>
